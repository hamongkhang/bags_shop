...
// if (!is_array($options) || count($options) === 0) {
    $defaults = get_appmakercheckout_settings();
    $options = array();
...
// Get the steps
$steps = get_appmakercheckout_steps();
...
$show_login_step = false; //(is_user_logged_in() || 'no' === get_option('woocommerce_enable_checkout_login_reminder')) ? false : true;
...
$swap_payment_review = apply_filters('appmakercheckout_swap_payment_review', $swap_payment_review);
...
$steps = apply_filters('appmakercheckout_modify_steps', $steps);

// Sort the steps
uasort($steps, 'appmakercheckout_sort_by_position');
...
<div class="appmakercheckout-steps-wrapper">

    <?php wc_print_notices(); ?>

    <div id="checkout_coupon" class="woocommerce_checkout_coupon" style="display: none;">
        <?php do_action('appmakercheckout-woocommerce_checkout_coupon_form', $checkout); ?>
    </div>
    ...
    <?php
    if ($show_login_step) {
        appmakercheckout_step_content_login($checkout, $stop_at_login);
    }

    if ($stop_at_login) {
        echo '</div>'; // closes the "appmakercheckout-steps-wrapper" div 
        return false;
    }
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url($checkout_url); ?>" enctype="multipart/form-data">
        ...
        <?php foreach ($steps as $_id => $_step) {
            echo "\n\n" . '<!-- Step: ' . $_step['title'] . ' -->' . "\n\n";
            echo '<div class="appmakercheckout-step-item ' . $_step['class'] . '">';
            if (isset($_step['sections'])) {
                foreach ($_step['sections'] as $_section) {
                    ...
                    if (strpos($_section, 'woocommerce_') === 0) {
                        ...
                        do_action('appmakercheckout_step_content_' . $_section);
                    }
                }
            } else {
                do_action('appmakercheckout_step_content_' . $_id);
            }
            echo '</div>';
            echo "\n\n" . '<!-- End Step: ' . $_step['title'] . ' -->' . "\n\n";
        } ?>
    </form>
