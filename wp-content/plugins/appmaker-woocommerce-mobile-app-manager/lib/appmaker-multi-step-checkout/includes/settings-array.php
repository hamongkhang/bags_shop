...
if ( ! function_exists( 'get_appmakercheckout_settings' ) ) {

	/**
	 * The settings array for the admin page.
	 */
	function get_appmakercheckout_settings() {

		$account_url                      = admin_url( 'admin.php?page=wc-settings&tab=account' );
		$no_login_screenshot              = '';
		$registration_settings_screenshot = '';
		$appmakercheckout_settings                    = array(
			/* General Settings */
			'label1'                   => array(
			...
			'appmakercheckout_buttons'             => array(
			...
			'appmakercheckout_check_sign'          => array(
			...
			't_previous'               => array(
				...
				'input_form' => 'input_text',
				'value'      => __( 'Previous', 'woocommerce' ),
				'section'    => 'titles',
			),
			't_next'                   => array(
				'label'      => __('Next', 'wp-multi-step-checkout' ),
				'input_form' => 'input_text',
				'value'      => __( 'Next', 'woocommerce' ),
				'section'    => 'titles',
			),
			't_error'                  => array(
		...
		return $appmakercheckout_settings;

	}
}

if ( ! function_exists( 'get_appmakercheckout_steps' ) ) {

	/**
	 * The steps array.
	 * Note: The Login is always the first step and is not part of the get_appmakercheckout_steps() array.
	 */
	function get_appmakercheckout_steps() {

		$steps = array(
			'billing'  => array(
				'title'    => __( 'Billing', 'wp-multi-step-checkout' ),
				'position' => 10,
				'class'    => 'appmakercheckout-step-billing',
				'sections' => array( 'billing' ),
			),
			'shipping' => array(
				'title'    => __( 'Shipping', 'wp-multi-step-checkout' ),
				'position' => 20,
				'class'    => 'appmakercheckout-step-shipping',
				'sections' => array( 'shipping' ),
			),
			'review'   => array(
				'title'    => __( 'Order', 'wp-multi-step-checkout' ),
				'position' => 30,
				'class'    => 'appmakercheckout-step-review',
				'sections' => array( 'review' ),
			),
			'payment'  => array(
				'title'    => __( 'Payment', 'wp-multi-step-checkout' ),
				'position' => 40,
				'class'    => 'appmakercheckout-step-payment',
...
if ( ! function_exists( 'appmakercheckout_step_content_login' ) ) {
	 ...
	 */
	function appmakercheckout_step_content_login( $checkout, $stop_at_login ) { ?> 
	<div class="appmakercheckout-step-item appmakercheckout-step-login">
...
if ( ! function_exists( 'appmakercheckout_step_content_billing' ) ) {

	/**
	 * The content of the Billing step.
	 */
	function appmakercheckout_step_content_billing() {
		do_action( 'woocommerce_checkout_before_customer_details' );
		do_action( 'woocommerce_checkout_billing' );
	}
}

if ( ! function_exists( 'appmakercheckout_step_content_shipping' ) ) {

	/**
	 * The content of the Shipping step.
	 */
	function appmakercheckout_step_content_shipping() {
		do_action( 'woocommerce_checkout_shipping' );
		do_action( 'woocommerce_checkout_after_customer_details' );
	}
}

if ( ! function_exists( 'appmakercheckout_step_content_payment' ) ) {

	/**
	 * The content of the Order Payment step.
	 */
	function appmakercheckout_step_content_payment() {
		echo '<h3 id="payment_heading">' . esc_html__( 'Payment', 'woocommerce' ) . '</h3>';
		do_action( 'appmakercheckout-woocommerce_checkout_payment' );
		do_action( 'woocommerce_checkout_after_order_review' );
	}
}

if ( ! function_exists( 'appmakercheckout_step_content_review' ) ) {

	/**
	 * The content of the Order Review step.
	 */
	function appmakercheckout_step_content_review() {
		do_action( 'woocommerce_checkout_before_order_review' );
		echo '<h3 id="order_review_heading">' . esc_html__( 'Your order', 'woocommerce' ) . '</h3>';
		echo '<div id="order_review">';
		do_action( 'woocommerce_checkout_order_review' );
		do_action( 'appmakercheckout-woocommerce_order_review' );
		echo '</div>';
	}
}

if ( ! function_exists( 'appmakercheckout_step_content_payment_germanized' ) ) {

	/**
	 * The content of the Payment step for the "Germanized for WooCommerce" plugin.
	 */
	function appmakercheckout_step_content_payment_germanized() {
		echo '<h3 id="payment_heading">' . esc_html__( 'Choose a Payment Gateway', 'woocommerce-germanized' ) . '</h3>';
		do_action( 'appmakercheckout-woocommerce_checkout_payment' );
	}
}

if ( ! function_exists( 'appmakercheckout_step_content_review_germanized' ) ) {

	/**
	 * The content of the Order Review step for the "Germanized for WooCommerce" plugin.
	 */
	function appmakercheckout_step_content_review_germanized() {
		do_action( 'woocommerce_checkout_before_order_review' );
		echo '<h3 id="order_review_heading">' . esc_html__( 'Your order', 'woocommerce' ) . '</h3>';
		do_action( 'appmakercheckout-woocommerce_order_review' );
...
if ( ! function_exists( 'appmakercheckout_step_content_review_german_market' ) ) {

	/**
	 * The content of the Order Review step for the "German Market" plugin.
	 */
	function appmakercheckout_step_content_review_german_market() {
		do_action( 'woocommerce_checkout_before_order_review' );
		echo '<h3 id="order_review_heading">' . esc_html__( 'Your order', 'woocommerce' ) . '</h3>';
		do_action( 'appmakercheckout-woocommerce_order_review' );
		do_action( 'woocommerce_checkout_order_review' );
...
if ( ! function_exists( 'appmakercheckout_sort_by_position' ) ) {
	 ...
	 */
	function appmakercheckout_sort_by_position( $a, $b ) {
