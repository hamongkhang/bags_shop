jQuery(window).ready(function($){
	...
	var appmakercheckout_checkout = {
		...
		init: function() {
			...
			$( '#appmakercheckout-submit' ).on( 'click', function() {
				$("#place_order").click();
				console.log("Clicked");
			});
		},
		...
		switch_tab: function( theIndex ) {
			...
			$( 'html, body' ).promise().done( function() {
				...
				// Last section
				if ( theIndex === self.$sections.length - 1 ) {
					$( '#appmakercheckout-submit' ).text($( '#place_order' ).text());
					$( '#appmakercheckout-next' ).hide( );
					$( '#appmakercheckout-submit' ).show( );
					...
					$( '#appmakercheckout-submit' ).hide( );
					$( '#appmakercheckout-next' ).show( );
				}
