...
class APPMAKER_WP_WC_REST_BACKEND_INAPPPAGE_Controller extends APPMAKER_WP_WC_REST_Controller {
	...
	public function create_item( $request ) {
		...
		$appData = APPMAKER_WP_WC_Converter::convert_inAppPage_data( $data, $key );
	...
	public function update_item( $request ) {		
		...
		if ( ! $option && ! ( isset( $data->is_new ) && $data->is_new = true ) ) {
		...
		if ( ( isset( $data->is_new ) && $data->is_new = true ) ) {
			unset( $data->is_new );
			if ( $option != false && empty($request['force_save'])) {
				$saveData = false;
			}
		}
		...
		if ( ! ( isset( $data->language ) && $data->language != 'default' ) ) {
			$savedKeys         = get_option( $this->getSafeKey( $this->inAppPagesKey ), array() );
			$savedKeys[ $key ] = array(
				'id'    => $data->id,
				'label' => $data->title,
