...
class Hybrid_Storage implements Hybrid_Storage_Interface {
	...
	public function config($key, $value = null) {
		...
		if ($value) {
			$GLOBALS["HA::CONFIG"][$key] = serialize($value);
		} elseif (isset($GLOBALS["HA::CONFIG"][$key])) {
			return unserialize($GLOBALS["HA::CONFIG"][$key]);
		}
	...
	public function get($key) {
		...
		if (isset($GLOBALS["HA::STORE"], $GLOBALS["HA::STORE"][$key])) {
			return unserialize($GLOBALS["HA::STORE"][$key]);
		}
	...
	public function set($key, $value) {
		$key = strtolower($key);
		$GLOBALS["HA::STORE"][$key] = serialize($value);
	}
	...
	function deleteMatch($key) {
		...
		if (isset($GLOBALS["HA::STORE"]) && count($GLOBALS["HA::STORE"])) {
			$f = $GLOBALS['HA::STORE'];
			foreach ($f as $k => $v) {
				if (strstr($k, $key)) {
					unset($f[$k]);
	...
	function getSessionData() {
		if (isset($GLOBALS["HA::STORE"])) {
			return serialize($GLOBALS["HA::STORE"]);
		}
		return null;
	}
	...
	function restoreSessionData($sessiondata = null) {
		$GLOBALS["HA::STORE"] = unserialize($sessiondata);
	}
