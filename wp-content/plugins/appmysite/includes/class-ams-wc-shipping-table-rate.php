...
class AMS_WC_Shipping_Table_Rate {
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return parent_get_option( $key, $empty_value );
	}

	public function parent_get_option( $key, $empty_value = null ) {
		...
		// Return global option.
		$option = apply_filters( 'woocommerce_shipping_' . $this->id . '_option', parent_get_option( $key, $empty_value ), $key, $this );
		return $option;
	}
	/**
	...
	public function parent_process_admin_options() {
		...
		echo("Working-12");
		if ( ! $this->instance_id ) {
			return false;//return parent::process_admin_options();
		}
	...
	public function count_items_in_class( $package, $class_id ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			//$values['data'] = wc_get_product($values['product_id']);
			if(isset($values['variation_id'])){
				$values['data'] = wc_get_product($values['variation_id']);  //modified
			}else{
				$values['data'] = wc_get_product($values['product_id']);  //modified
			}
	...
	public function get_cart_shipping_class_id( $package ) {
		...
		if ( sizeof( $package['contents'] ) > 0 ) {
			...
			foreach ( $package['contents'] as $item_id => $values ) {
				...
				//	$values['data'] = $values['product_id'];  //modified
	...
	public function get_rates( $package ) {
		...
		if ( $this->calculation_type == 'item' ) {
			...
			foreach ( $package['contents'] as $item_id => $values ) {
				...
				if(isset($values['variation_id'])){
					$_product = wc_get_product($values['variation_id']);  //modified
				}else{
					$_product = wc_get_product($values['product_id']);  //modified
				}
		...
		$is_customer_vat_exempt = false; //WC()->cart->get_customer()->get_is_vat_exempt();
	...
	private function get_fee( $fee, $total ) {   //added
	...
	private function is_taxable() { //added and modified
